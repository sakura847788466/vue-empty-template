    ...
    <div class="left">
      <div class="option-list">
        ...
        <div class="option-btn">
          ...
          <el-button type="primary" disabled>添加区域名称</el-button>
        </div>
        <el-dialog
          title="添加站点"
      ...
      <div class="option-list">
        ...
        <div class="option-btn">
          <el-button type="primary" @click="openThpyoon">绘制台风</el-button>
          <el-button type="primary" @click="clearAllTyphoon">清除台风图层</el-button>
          <!-- 台风列表 -->
          <el-dialog
            title="台风列表"
            :visible.sync="dialogVisibleTyphoon"
            width="30%"
            custom-class="typhoonDialog"
            :before-close="handleCloseTyphoon"
          >
            <div class="dialog-container">
              <el-table :data="tableData" class="typhoon-table-list" style="width: 100%">
                <el-table-column width="60">
                  <template slot-scope="scope">
                    <el-checkbox
                      v-model="scope.row.checked"
                      @change="chooseTyphoon(scope.row,scope.$index)"
                    ></el-checkbox>
                  </template>
                </el-table-column>
                <el-table-column prop="tfbh" label="编号"></el-table-column>
      ...
      <div class="option-list">
        ...
        <div class="option-btn">
          ...
          <el-button
            type="primary"
            @click="geoJsonDraw"
          >{{geoJsonStatus?'geoJson区域移除':'geoJson区域描边'}}</el-button>
        </div>
      </div>
      <div class="option-list">
        <div class="title">
        ...
        <div class="option-btn">
          <el-button
            type="primary"
            @click="addWindyAllGlobal"
          >{{windGlobalStatus?'移除风场效果(全球)':'添加风场效果(全球)'}}</el-button>
          <el-button
            type="primary"
            @click="addWindyOnlyPart"
      ...
      <div class="nav-option">
        <ul>
          <li
            ...
            :key="index"
            @click="item.clickEvent(index)"
            :class="{'isActive':currentActive === index}"
          >
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 台风数据
      tableData: [],
      typhoonObject:[],//台风加载列表
      dialogVisibleTyphoon: false,
      // 弹窗
      dialogVisible: false,
      currentActive: null,
      rules: {
  ...
  methods: {
    ...
    addRectangle() {
      areaData.data.forEach(item => {
        ...
        let rectangle = L.rectangle(itemLatandLng, {
          color: "#ff7800",
          weight: 2
        })
        ...
        L.marker([rectangle.getCenter().lat, rectangle.getCenter().lng], {
          icon: myIcon
        }).addTo(this.map);
      });
    },
    ...
    async getThpyoon(number) {
      let num = number || 2021;
      let allList = [];
      let listData = (
        await Axios.get(
          `http://data.istrongcloud.com/v2/data/complex/${num}.json?v=1634543270108`
        )
      ).data;
    ...
    chooseTyphoon(row) {
      if (row.checked) {
        // 添加对应天风路径
        this.addTyphoon(row.tfbh)
      }else {
        this.deleteTyphoon(row.tfbh)
      }
    },
     // 删除台风
    deleteTyphoon(number) {
      this.typhoonObject[number].deleteTyphoon();
    },
    // 添加台风路径
    async addTyphoon(number) {
      // 温州台风网数据
      let typhoonData = (
        await Axios.get(
          `http://data.istrongcloud.com/v2/data/complex/${number}.json?v=1640162690433`
        )
      ).data;
      let arr = [
        { label: "日本", isShow: true },
        { label: "中国", isShow: true },
        { label: "中国台湾", isShow: true },
        { label: "美国", isShow: true }
      ];
      let typhoonOne = new typhoon(
        this.map,
        L,
        number,
        typhoonData,
        true,
        number => {
          this.typhoonObject[number] = typhoonOne;
          // 添加预测路径
          arr.forEach(item => {
            this.typhoonObject[number].forecastingAgencyIsShow(
              item.label,
              item.isShow
            );
          });
    ...
    async addWindyAllGlobal() {
      if (this.velocityLayer) {
        ...
        let windyData = (
          await Axios.get(
            `https://datacenter.istrongcloud.com/data/gfs/fcdata/202112/20/08/006.json?v=1639982791935`
          )
        ).data["2021122014"];
    ...
    handleCloseTyphoon() {
      this.dialogVisibleTyphoon = false
    },
    // 清除所有台风
    clearAllTyphoon() {
